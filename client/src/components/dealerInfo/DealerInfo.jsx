...
export const DealerInfo = ({ showDeleteButton = true, showLabel = true }) => {
  ...
  const matchesSearchTerm = (code) => {
    ...
    return (
      code.LOIreferenceId?.toLowerCase().includes(searchValue) ||
      code.RQreferenceId?.toLowerCase().includes(searchValue) ||
      code.activationCode?.toLowerCase().includes(searchValue) ||
      code.dealerCode?.toLowerCase().includes(searchValue) ||
      code.name?.toLowerCase().includes(searchValue) ||
      code.mobileNo?.toLowerCase().includes(searchValue) ||
      code.emailId?.toLowerCase().includes(searchValue) ||
      code.gstin?.toLowerCase().includes(searchValue) ||
      code.address?.toLowerCase().includes(searchValue) ||
      code.state?.toLowerCase().includes(searchValue) ||
      code.dist?.toLowerCase().includes(searchValue) ||
      code.pinCode?.toLowerCase().includes(searchValue) ||
      code.rtoOffice?.toLowerCase().includes(searchValue)
    );
  };

  // Updated search logic: filtering by all fields
  ...
  const fetchActivationCodes = async () => {
    ...
    setLoading(true);
    try {
      const response = await axios.get(getApiUrl("/dealer/full"));
  ...
  const handleDelete = async (codeToDelete) => {
    try {
      await axios.delete(
        getApiUrl(`/dealer/${encodeURIComponent(codeToDelete)}`)
      );
                ...
                <TableBody>
                  {filteredCodes.map((code) => (
                    <TableRow key={code.code || code.id}>
                      ...
                      <TableCell align="center">{code.RQreferenceId}</TableCell>
                      <TableCell align="center">
                        {code.activationCode}
                      </TableCell>
                      <TableCell align="center">{code.dealerCode}</TableCell>
                      <TableCell align="center">{code.name}</TableCell>
                      <TableCell align="center">{code.mobileNo}</TableCell>
                      <TableCell align="center">{code.emailId}</TableCell>
                      <TableCell align="center">{code.gstin}</TableCell>
                      ...
                      <TableCell align="center">{code.rtoOffice}</TableCell>
                      {showDeleteButton && (
                      <TableCell align="center">
